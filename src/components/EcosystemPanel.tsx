...
export default function EcosystemPanel({ onClose }: EcosystemPanelProps) {
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center pointer-events-auto bg-black/60 backdrop-blur-sm">
      <div className="w-[600px] max-h-[80vh] bg-black/90 border border-green-500/30 rounded-xl p-6 flex flex-col shadow-[0_0_30px_rgba(0,255,0,0.15)]">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-3">
          {filteredApps.map(app => (
            <div key={app.id} className="bg-black/40 border border-green-500/20 rounded-lg p-4 hover:border-green-500/40 transition-colors group">
              <div className="flex justify-between items-start mb-2">
                <div>
                  <h3 className="text-lg font-bold text-gray-200 group-hover:text-green-400 transition-colors">{app.name}</h3>
                  <div className="flex gap-2 mt-2">
                    <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-green-500/10 text-green-400/70 border border-green-500/20">
                      {app.platform}
                    </span>
                ...
                <a href={app.url} target="_blank" rel="noreferrer" className="text-gray-500 hover:text-green-400 transition-colors">
