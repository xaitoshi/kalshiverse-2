...
function createSatelliteObject(): THREE.Object3D {
  ...
  const panelMat = new THREE.MeshBasicMaterial({ color: 0x0a3d0a });
  ...
  const antMat = new THREE.MeshBasicMaterial({ color: 0x00cc00 });
  ...
  const dishMat = new THREE.MeshBasicMaterial({ color: 0x00ff44, transparent: true, opacity: 0.7 });
  ...
  return (
    <div className="absolute inset-0 z-0">
      <Globe
        ...
        pointLat="lat"
        pointLng="lng"
        pointColor={(d: object) => (d as MarketData).id.startsWith('kalshi-') ? '#00FF00' : '#3B82F6'}
        pointAltitude={0.005}
        pointRadius="size"
        pointsMerge={false}
        ...
        ringColor={(d: object) => {
          ...
          if (ring.type === 'kalshi') return (t: number) => `rgba(0, 255, 0, ${1 - t})`;
      ...
      {nearbyMarket && satScreenPos && (
        ...
        >
          <div className="bg-black/90 border border-green-500/40 backdrop-blur-xl rounded-lg px-4 py-3 shadow-[0_0_20px_rgba(0,255,0,0.15)]">
            <div className="flex items-center gap-2 mb-2">
              <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse" />
              <span className="text-green-400 font-mono font-bold text-sm">{nearbyMarket.title}</span>
            </div>
            <div className="space-y-1.5">
              {nearbyMarket.markets.slice(0, 3).map((m, i) => (
                <div key={i} className="flex items-start justify-between gap-2">
                  ...
                  <span className="text-green-400 font-mono text-[11px] shrink-0">
                    {Math.round(m.yesPrice * 100)}¢
                  </span>
                </div>
              ))}
            </div>
            <div className="mt-2 pt-1.5 border-t border-green-500/20 text-gray-500 font-mono text-[9px]">
              {nearbyMarket.news}
            </div>
          </div>
        </div>
