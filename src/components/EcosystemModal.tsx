...
export default function EcosystemModal({ onClose, onOpenAdmin }: EcosystemModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="w-full max-w-4xl bg-[#0a0a0a] border border-green-500/30 rounded-xl shadow-[0_0_40px_rgba(0,255,0,0.1)] flex flex-col max-h-[85vh]">
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredProjects.map(project => (
              <div key={project.id} className="bg-black/40 border border-green-500/20 rounded-xl p-5 hover:border-green-500/50 transition-colors group flex flex-col">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className="text-[10px] uppercase tracking-wider font-mono px-2 py-1 rounded bg-green-500/10 text-green-400 border border-green-500/20">
                    {project.platform}
                  </span>
                </div>
                
                <h3 className="text-lg font-bold text-white mb-2 group-hover:text-green-400 transition-colors">{project.name}</h3>
                ...
                <div className="flex items-center justify-between mt-auto pt-4 border-t border-gray-800">
                  <div className="flex flex-col">
                    ...
                    {project.tokenLink ? (
                      <a href={project.tokenLink} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-green-400 hover:text-green-300 transition-colors">{project.token || 'N/A'}</a>
                    ) : (
                  ...
                  <a 
                    ...
                    className="flex items-center gap-1.5 text-sm font-medium text-green-400 hover:text-green-300 transition-colors bg-green-500/10 px-3 py-1.5 rounded-lg"
                  >
